...
app.post("/register", async (req, res) => {
  ...
  const cleanUsername = sanitizeHtml(username, {
    allowedTags: [], // Inga HTML-taggar tillåtna i titeln
    allowedAttributes: {}, // Inga tillåtna HTML-attribut
  });

  // Sanitizera innehållet
  const cleanPassword = sanitizeHtml(password, {
    allowedTags: ["b", "u", "i", "strong", "br", "em"], // Tillåtna HTML-taggar för innehållet
    allowedAttributes: {}, // Inga tillåtna HTML-attribut
  });
  try {
...
app.post("/addpost", authenticate, verifyCsrfToken, async (req, res) => {
  ...
  // Kontrollera om titel eller innehåll saknas
  if (!title || !content) {
    return res
      .status(400)
...
//la till detta den 22/1-24 för att användare ska kunna kommentera varandras inlägg
// Lägg till en kommentar för ett specifikt blogginlägg
app.post(
  "/addcomment/:postId",
  authenticate,
  verifyCsrfToken,
  async (req, res) => {
    ...
    const cleanContent = sanitizeHtml(content, {
      allowedTags: ["b", "u", "i", "strong", "br", "em"], // tillåtna HTML-taggar
      allowedAttributes: {}, // Inga tillåtna HTML-attribut
    });
    try {
      await pool.query(
        "INSERT INTO comments (post_id, username, content) VALUES ($1, $2, $3)",
        [postId, author, cleanContent]
      );
